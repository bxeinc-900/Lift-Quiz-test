  ...
  header.innerHTML = `
    <div class="container">
      <div class="site-header">
        <div class="site-logo">AFI GROUP<span>RETIREMENT STRATEGIES</span></div>
        <button class="btn btn-outline" onclick="window.openBookingModal()" style="padding: 0.5rem 1rem; font-size: 0.8rem;">SCHEDULE CONSULTATION</button>
      </div>
    </div>
  `
...
function renderHero() {
  ...
  section.innerHTML = `
    ...
    <h1>Is Your Retirement Plan <span class="text-gold">Maximizing What You Keep?</span></h1>
    <p>
    ...
    <div class="hero-btns">
      <button id="start-quiz-btn" class="btn btn-primary" onclick="window.startQuiz()">START FREE ASSESSMENT</button>
      <button id="learn-method-btn" class="btn btn-outline" onclick="window.openMethodModal()">LEARN THE METHOD</button>
    </div>
    <p style="font-size: 0.8rem; color: #495670; margin-top: 1.5rem;">
...
function renderResults() {
  ...
  section.innerHTML = `
    <div class="results-header">
      ...
      <h1>Risk Profile: <span style="color: ${exposureColor};">${exposure}</span></h1>
      ...
      <p style="font-size: 0.8rem; color: #495670; margin-top: 0.75rem; max-width: 700px; margin-left: auto; margin-right: auto;">
        <em>For illustrative and educational purposes only. Not a formal financial assessment.</em>
      </p>
    </div>
    ...
    <section class="calculator-section">
      <div style="text-align: center; margin-bottom: 1.5rem;">
        <div style="height: 2px; width: 48px; background: hsl(var(--accent)); margin: 0 auto 1.25rem;"></div>
        <h2 style="margin-bottom: 0.75rem;">Illustrative Retirement Calculators</h2>
        <p style="color: #8892B0; font-size: 0.95rem;">Enter your numbers to explore hypothetical scenarios.</p>
      </div>
      ...
      <p class="calc-disclaimer" style="text-align:center;">
    ...
    <div class="cta-section">
      <div class="cta-banner glass">
        <div class="glass-pill" style="margin-bottom: 1.5rem;">NEXT STEP: PERSONALIZED ANALYSIS</div>
        <h2>See How the LIFT Method <span class="text-gold">Applies to Your Situation</span></h2>
        <div style="background: rgba(2, 12, 27, 0.6); border-radius: 12px; padding: 1.5rem; margin: 1.5rem 0; border: 1px solid rgba(255,255,255,0.05); text-align: left;">
          <p style="color: #CCD6F6; font-size: 1rem; line-height: 1.6; margin-bottom: 1.25rem;">
            Based on your assessment, there may be meaningful opportunities to improve your retirement income strategy. A complimentary strategy session will explore your specific situation in detail.
          </p>
          <div class="cta-meta">
            <div>
              <span style="display: block; font-size: 0.72rem; color: #8892B0; text-transform: uppercase; letter-spacing: 1px; margin-bottom: 0.25rem;">Session Type</span>
              <span class="value-green" style="font-weight: 700;">Complimentary</span>
            </div>
            <div>
              <span style="display: block; font-size: 0.72rem; color: #8892B0; text-transform: uppercase; letter-spacing: 1px; margin-bottom: 0.25rem;">Duration</span>
              <span style="color: #E6F1FF; font-weight: 700;">30–45 Minutes</span>
            </div>
            <div>
              <span style="display: block; font-size: 0.72rem; color: #8892B0; text-transform: uppercase; letter-spacing: 1px; margin-bottom: 0.25rem;">Commitment</span>
              <span class="text-gold" style="font-weight: 700;">Zero Obligation</span>
            </div>
          </div>
        </div>
        <button class="btn btn-primary" onclick="window.openBookingModal()" style="width: 100%; max-width: 420px; font-size: 1.05rem; padding: 1.1rem 2rem;">SCHEDULE YOUR COMPLIMENTARY CALL</button>
        <p style="margin-top: 1.25rem; color: #8892B0; font-size: 0.85rem;">No products will be sold. This is a complimentary educational consultation only.</p>
      </div>
    </div>
  `
  return section
...
function renderActiveCalculator() {
  if (activeCalcTab === 'tax') {
    ...
    return `
      <div class="calc-layout">
        <div>
          <h3 style="margin-bottom: 1.25rem; font-size: 1.1rem;">Your Inputs</h3>
          <label class="calc-input-label">Current retirement balance</label>
          <div class="calc-input-wrap">
            <span class="calc-input-symbol">$</span>
            <input class="calc-input" type="number" value="${calcData.balance}" onchange="window.calcData.balance = Number(this.value); window.render()">
          </div>
          <label class="calc-input-label">Annual withdrawal amount</label>
          <div class="calc-input-wrap">
            <span class="calc-input-symbol">$</span>
            <input class="calc-input" type="number" value="${calcData.withdrawal}" onchange="window.calcData.withdrawal = Number(this.value); window.render()">
          </div>
          <label class="calc-input-label">Assumed tax bracket in retirement</label>
          <div class="calc-input-wrap">
            <select class="calc-select" onchange="window.calcData.bracket = Number(this.value); window.render()">
          ...
          <p class="calc-note">2026 federal marginal brackets shown for reference. Actual taxes may be lower due to standard deductions. State taxes not included.</p>
        </div>
        <div>
          <div class="result-card result-card-red">
            ...
            <p class="calc-note">${withdrawalRate}% of $${fmt(calcData.withdrawal)} withdrawal (marginal rate)</p>
          </div>
          <div class="result-card result-card-green">
            ...
            <p class="calc-note">After estimated federal income tax only</p>
          </div>
          <div class="result-card result-card-red">
            <div class="result-label">HYPOTHETICAL TOTAL TAX OVER 25 YEARS</div>
            <div class="result-value value-red">$${fmt(totalTaxes25yr)}</div>
            <p class="calc-note">Assumes same withdrawal &amp; bracket each year. No RMDs, no state tax, no inflation adjustment.</p>
          </div>
        </div>
      </div>
    `
  ...
  if (activeCalcTab === 'income') {
    ...
    return `
      <div class="calc-layout">
        <div>
          <h3 style="margin-bottom: 0.75rem; font-size: 1.1rem;">The Income Gap Illustration</h3>
          <p style="color: #8892B0; margin-bottom: 1.25rem; font-size: 0.9rem; line-height: 1.6;">How much more will you need to withdraw in 20 years to keep the same take-home income — after inflation and taxes?</p>
          <label class="calc-input-label">Annual net income you want in retirement</label>
          <div class="calc-input-wrap">
            <span class="calc-input-symbol">$</span>
            <input class="calc-input" type="number" value="${calcData.withdrawal}" onchange="window.calcData.withdrawal = Number(this.value); window.render()">
          </div>
          ...
          <div class="calc-disclaimer">Assumes: 3% annual inflation × 20 years (≈1.81×). Tax applied at selected bracket. State taxes not included.</div>
        </div>
        <div>
          <div class="result-card result-card-red">
            <div class="result-label">ANNUAL TAX DRAG (TODAY)</div>
            <div class="result-value value-red">$${fmt(taxDragToday)}</div>
            <p class="calc-note">${calcData.bracket}% on $${fmt(calcData.withdrawal)} withdrawal</p>
          </div>
          <div class="result-card result-card-red">
            <div class="result-label">INFLATION GAP AFTER 20 YEARS</div>
            <div class="result-value value-red">+$${fmt(inflationOnlyGap)}/yr</div>
            <p class="calc-note">Additional needed just to match inflation (3% × 20 yrs)</p>
          </div>
          <div class="result-card result-card-gold">
            <div class="result-label text-gold">GROSS WITHDRAWAL NEEDED IN YEAR 20</div>
            <div class="result-value">$${fmt(grossNeededYear20)}</div>
            <p class="calc-note">To net same as $${fmt(calcData.withdrawal)}/yr today, after ${calcData.bracket}% tax + inflation</p>
          </div>
        </div>
      </div>
    `
  ...
  if (activeCalcTab === 'side') {
    return `
      <div>
        <h3 style="text-align: center; margin-bottom: 1.5rem; font-size: 1.1rem;">Hypothetical Comparison: Traditional 401(k) vs. LIFT Strategy</h3>
        <div class="calc-pair" style="margin-bottom: 1.25rem;">
          <div class="result-card result-card-red">
            <h3 class="value-red" style="margin-bottom: 1rem; font-size: 1rem;">Traditional 401(k) / IRA</h3>
            <ul style="list-style: none; color: #8892B0; font-size: 0.9rem; line-height: 1.9;">
              <li>⚠️ All withdrawals taxed as ordinary income</li>
              <li>⚠️ RMDs required at age 73+</li>
              <li>⚠️ 100% exposed to market downturns</li>
              <li>⚠️ Tax rate set by Congress — may change</li>
              <li>⚠️ No death benefit</li>
            </ul>
          </div>
          <div class="result-card result-card-green" style="border-left: 3px solid hsl(var(--accent));">
            <h3 class="value-green" style="margin-bottom: 1rem; font-size: 1rem;">LIFT Strategy (Properly Structured)</h3>
            <ul style="list-style: none; color: #ccd6f6; font-size: 0.9rem; line-height: 1.9;">
              <li>✅ Loans can be tax-free (if structured properly)</li>
              <li>✅ No RMD requirements</li>
              <li>✅ Indexed growth with 0% floor on downturns*</li>
              <li>✅ Death benefit provides legacy</li>
              <li>✅ No contribution limits after funding phase</li>
            </ul>
          </div>
        </div>
        <div class="calc-disclaimer">*The 0% floor applies to indexed interest crediting only — not to total cash value, which is reduced by ongoing charges and fees. Tax-free loans assume the product remains in force. This is a simplified illustration. Actual results depend on many factors. Not a product recommendation.</div>
      </div>
    `
  }
  ...
  if (activeCalcTab === 'risk') {
    ...
    return `
      <div>
        <div class="calc-summary-chips">
          <div class="calc-chip">BALANCE <strong>$${fmt(calcData.balance)}</strong></div>
          <div class="calc-chip">WITHDRAWAL <strong>$${fmt(calcData.withdrawal)}/yr</strong></div>
          <div class="calc-chip">BRACKET <strong>${calcData.bracket}%</strong></div>
        </div>
        <div class="calc-inline-inputs">
          <div class="calc-inline-field">
            <label>Balance:</label>
            <input type="number" value="${calcData.balance}" onchange="window.calcData.balance = Number(this.value); window.render()">
          </div>
          <div class="calc-inline-field">
            <label>Tax bracket:</label>
            <select onchange="window.calcData.bracket = Number(this.value); window.render()">
              <option value="12" ${calcData.bracket === 12 ? 'selected' : ''}>12%</option>
              <option value="22" ${calcData.bracket === 22 ? 'selected' : ''}>22%</option>
              <option value="24" ${calcData.bracket === 24 ? 'selected' : ''}>24%</option>
              <option value="32" ${calcData.bracket === 32 ? 'selected' : ''}>32%</option>
        ...
        <h3 style="text-align: center; margin-bottom: 0.75rem; font-size: 1.1rem;">Hypothetical Sequence of Returns Scenario</h3>
        <p style="text-align: center; color: #8892B0; margin-bottom: 1.25rem; font-size: 0.875rem; line-height: 1.5;">A market decline early in retirement is especially damaging — you sell depreciated assets to fund withdrawals, preventing full recovery.</p>
        <div class="calc-pair" style="margin-bottom: 1rem;">
          <div class="result-card result-card-red">
            <div class="result-label">TRADITIONAL ACCOUNT AFTER 20% CRASH</div>
            <div class="result-value value-red">$${fmt(crashValue)}</div>
            <p class="calc-note">Loss: $${fmt(crashLoss)}</p>
            <p class="calc-note">Recovery time (7%/yr): ~${yearsToRecover} years</p>
            <p class="calc-note" style="color: #F44336;">Effective loss incl. withdrawals: ~$${fmt(effectiveLossWithWithdrawal)}</p>
          </div>
          <div class="result-card result-card-green">
            <div class="result-label">PROPERLY STRUCTURED PRODUCT — 0% FLOOR*</div>
            <div class="result-value value-green">$${fmt(calcData.balance)}</div>
            <p class="calc-note">Index credits: $0 (floored, not negative)</p>
            <p class="calc-note">Charges still reduce cash value — see disclosure</p>
            <p class="calc-note" style="color: #4CAF50;">No recovery period required</p>
          </div>
        </div>
        <div class="calc-pair" style="margin-bottom: 1rem;">
          <div class="result-card result-card-gold">
            <div class="result-label text-gold">ANNUAL TAX ON WITHDRAWALS</div>
            <div class="result-value">$${fmt(taxDragAnnual)}/yr</div>
            <p class="calc-note">From 401k at ${calcData.bracket}% bracket</p>
          </div>
          <div class="result-card result-card-teal">
            <div class="result-label value-teal">TAX ON PROPERLY STRUCTURED LOANS</div>
            <div class="result-value value-teal">$0/yr*</div>
            <p class="calc-note">Loans not taxable if product stays in force</p>
          </div>
        </div>
        <div class="calc-disclaimer">*Hypothetical only. 0% floor applies to indexed interest crediting only — not to cash value which is reduced by charges and fees. Loans reduce account value; if product lapses with outstanding loan balance, adverse tax consequences may result. Recovery time assumes 7%/yr avg return, no ongoing withdrawals. Not a guarantee of any outcome.</div>
      </div>
    `
  }
  return ''
